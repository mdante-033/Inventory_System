...
<head>
    ...
    <style>
        :root {
            ...
            --gold-pale: rgba(201,168,76,.12); --gold-pale-2: rgba(201,168,76,.07);
            --cream: #f5f0e8; --ink: #0e0c09; --ink-mid: #1c1810; --ink-soft: #2e2b24;
            --mist: rgba(245,240,232,.06); --mist-border: rgba(245,240,232,.12);
            --text-dim: rgba(245,240,232,.5); --text-mid: rgba(245,240,232,.75);
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /*
         * KEY FIX 1 — scroll-padding-top ensures anchor links
        ...
        .skip-link { position:absolute; top:-60px; left:1.5rem; background:var(--cream); color:var(--ink); padding:.6rem 1.2rem; border-radius:4px; font-weight:600; font-size:.85rem; z-index:9000; transition:top .2s; }
        .skip-link:focus { top:1rem; }

        /* HEADER */
        .site-header { position:fixed; top:0; left:0; right:0; z-index:800; height:var(--header-h); display:flex; align-items:center; padding:0; transition:background .4s,box-shadow .4s; }
        .site-header.scrolled { background:rgba(14,12,9,.95); backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px); border-bottom:1px solid var(--mist-border); box-shadow:0 4px 32px rgba(0,0,0,.4); }
        .header-inner { width:100%; display:flex; align-items:center; justify-content:space-between; }
        .logo { display:flex; align-items:center; gap:.7rem; transition:opacity .2s; }
        .logo:hover { opacity:.8; }
        .logo-mark { width:36px; height:36px; border:1.5px solid var(--gold); border-radius:50%; display:flex; align-items:center; justify-content:center; color:var(--gold); font-family:var(--serif); font-size:1rem; font-weight:600; flex-shrink:0; }
        .logo-name { font-family:var(--serif); font-size:1.45rem; font-weight:400; letter-spacing:.06em; }
        .logo-name em { font-style:italic; color:var(--gold); }
        .primary-nav { display:flex; align-items:center; gap:2.5rem; }
        .nav-link { font-size:.8rem; font-weight:500; letter-spacing:.12em; text-transform:uppercase; color:var(--text-mid); position:relative; padding-bottom:3px; transition:color .25s; }
        .nav-link::after { content:''; position:absolute; bottom:0; left:0; width:0; height:1px; background:var(--gold); transition:width .3s var(--ease-out); }
        .nav-link:hover { color:var(--cream); }
        .nav-link:hover::after { width:100%; }
        .contact-wrapper { position:relative; }
        .contact-btn { background:none; border:none; color:var(--text-mid); font-size:.8rem; font-weight:500; letter-spacing:.12em; text-transform:uppercase; padding-bottom:3px; position:relative; transition:color .25s; }
        .contact-btn::after { content:''; position:absolute; bottom:0; left:0; width:0; height:1px; background:var(--gold); transition:width .3s var(--ease-out); }
        .contact-btn:hover { color:var(--cream); }
        .contact-btn:hover::after { width:100%; }
        .contact-popover { position:absolute; top:calc(100% + 16px); right:0; width:230px; background:var(--ink-soft); border:1px solid var(--mist-border); border-radius:8px; padding:1.2rem 1.4rem; box-shadow:0 20px 60px rgba(0,0,0,.6); opacity:0; pointer-events:none; transform:translateY(-8px); transition:opacity .25s,transform .25s var(--ease-out); z-index:900; }
        .contact-popover.open { opacity:1; pointer-events:all; transform:translateY(0); }
        .cp-label { font-size:.7rem; letter-spacing:.14em; text-transform:uppercase; color:var(--gold); margin-bottom:.6rem; }
        .cp-name { font-family:var(--serif); font-size:1.3rem; margin-bottom:.15rem; }
        .cp-role { font-size:.78rem; color:var(--text-dim); margin-bottom:.9rem; }
        .cp-phone { display:inline-flex; align-items:center; gap:.4rem; font-size:.9rem; color:var(--gold-light); transition:color .2s; }
        .cp-phone:hover { color:var(--cream); }
        .auth-links { display:flex; align-items:center; gap:.9rem; }
        .btn-nav-link { font-size:.78rem; font-weight:500; letter-spacing:.14em; text-transform:uppercase; color:var(--text-mid); transition:color .3s; }
        .btn-nav-link:hover { color:var(--cream); }
        .btn-nav-primary { font-size:.78rem; font-weight:600; letter-spacing:.14em; text-transform:uppercase; border:1px solid var(--gold); color:var(--gold); padding:.55rem 1.4rem; border-radius:2px; transition:background .3s,color .3s; }
        ...
        .mobile-nav { display:none; position:fixed; top:0; right:-100%; bottom:0; width:min(340px,80vw); background:var(--ink-mid); border-left:1px solid var(--mist-border); z-index:700; padding:6rem 2.5rem 3rem; flex-direction:column; gap:1.5rem; transition:right .4s var(--ease-out); }
        .mobile-nav.open { right:0; }
        .mobile-nav a { font-size:1.15rem; font-weight:300; letter-spacing:.06em; color:var(--text-mid); border-bottom:1px solid var(--mist-border); padding-bottom:1rem; transition:color .2s; }
        .mobile-nav a:hover { color:var(--gold); }
        .nav-overlay { display:none; position:fixed; inset:0; background:rgba(0,0,0,.5); z-index:699; }
        .nav-overlay.open { display:block; }

        /* HERO */
        .hero { min-height:100vh; position:relative; display:flex; align-items:center; overflow:hidden; }
        .hero-bg { position:absolute; inset:0; background:url('https://images.unsplash.com/photo-1490481651871-ab68de25d43d?auto=format&fit=crop&w=2000&q=80') center/cover no-repeat; background-color:#1a1510; transform:scale(1.05); animation:heroZoom 12s ease-out forwards; }
        @keyframes heroZoom { from{transform:scale(1.05)} to{transform:scale(1)} }
        .hero-overlay { position:absolute; inset:0; background:linear-gradient(120deg,rgba(14,12,9,.92) 0%,rgba(14,12,9,.72) 55%,rgba(14,12,9,.4) 100%); }
        .hero-lines { position:absolute; inset:0; pointer-events:none; opacity:.12; }
        ...
        .hero-eyebrow { display:flex; align-items:center; gap:1rem; font-size:.72rem; letter-spacing:.25em; text-transform:uppercase; color:var(--gold); margin-bottom:2rem; opacity:0; animation:fadeUp .7s .3s var(--ease-out) forwards; }
        .hero-eyebrow::before { content:''; display:block; width:40px; height:1px; background:var(--gold); }
        .hero-title { font-family:var(--serif); font-size:clamp(3.5rem,7vw,6.5rem); font-weight:300; line-height:1.05; margin-bottom:1.8rem; opacity:0; animation:fadeUp .8s .5s var(--ease-out) forwards; }
        .hero-title em { font-style:italic; color:var(--gold-light); display:block; }
        .hero-desc { font-size:1.05rem; font-weight:300; color:var(--text-mid); line-height:1.75; max-width:460px; margin-bottom:2.8rem; opacity:0; animation:fadeUp .8s .7s var(--ease-out) forwards; }
        .hero-actions { display:flex; gap:1.2rem; flex-wrap:wrap; opacity:0; animation:fadeUp .8s .9s var(--ease-out) forwards; }
        .btn-cta { display:inline-flex; align-items:center; gap:.65rem; background:var(--gold); color:var(--ink); font-size:.78rem; font-weight:600; letter-spacing:.16em; text-transform:uppercase; padding:1rem 2rem; border-radius:2px; border:1px solid var(--gold); transition:background .3s,transform .2s; }
        .btn-cta:hover { background:var(--gold-light); transform:translateY(-2px); }
        .btn-cta svg { width:16px; height:16px; flex-shrink:0; }
        .btn-ghost { display:inline-flex; align-items:center; gap:.65rem; background:transparent; color:var(--cream); font-size:.78rem; font-weight:500; letter-spacing:.16em; text-transform:uppercase; padding:1rem 2rem; border:1px solid var(--mist-border); border-radius:2px; transition:border-color .3s,color .3s; }
        .btn-ghost:hover { border-color:var(--gold); color:var(--gold); }
        .hero-auth-note { margin-top:1.2rem; color:var(--text-mid); font-size:.92rem; opacity:0; animation:fadeUp .8s 1.05s var(--ease-out) forwards; }
        .hero-auth-note a { color:var(--gold-light); font-weight:500; }
        .hero-scroll { position:absolute; bottom:2.5rem; left:50%; transform:translateX(-50%); display:flex; flex-direction:column; align-items:center; gap:.5rem; font-size:.65rem; letter-spacing:.2em; text-transform:uppercase; color:var(--text-dim); z-index:2; animation:fadeIn 1s 1.5s both; }
        ...
        .flash-banner { position:fixed; top:calc(var(--header-h) + 12px); left:50%; transform:translateX(-50%); z-index:780; width:min(720px,calc(100% - 2rem)); padding:1rem 1.2rem; border-radius:6px; border:1px solid rgba(201,168,76,.28); background:rgba(24,22,17,.94); backdrop-filter:blur(16px); color:var(--cream); box-shadow:0 18px 45px rgba(0,0,0,.36); }
        .flash-banner strong { color:var(--gold-light); }

        /* MARQUEE */
        .marquee-strip { background:var(--gold); padding:.7rem 0; overflow:hidden; }
        .marquee-track { display:flex; white-space:nowrap; animation:marquee 28s linear infinite; }
        .marquee-item { display:inline-flex; align-items:center; gap:1.5rem; padding:0 2.5rem; font-size:.7rem; font-weight:600; letter-spacing:.18em; text-transform:uppercase; color:var(--ink); }
        .marquee-dot { width:4px; height:4px; border-radius:50%; background:var(--ink); opacity:.4; }
        @keyframes marquee { from{transform:translateX(0)} to{transform:translateX(-50%)} }

        /* SHARED */
        .section-label { display:inline-flex; align-items:center; gap:.8rem; font-size:.68rem; letter-spacing:.22em; text-transform:uppercase; color:var(--gold); margin-bottom:1.2rem; }
        ...
        .feature-card { padding:2.8rem 2.2rem; border-right:1px solid var(--mist-border); transition:background .35s; position:relative; }
        ...
        .feature-num { position:absolute; top:1.8rem; right:2rem; font-family:var(--serif); font-size:.75rem; color:var(--text-dim); }
        .feature-title { font-family:var(--serif); font-size:1.3rem; margin-bottom:.8rem; }
        .feature-desc { font-size:.88rem; color:var(--text-dim); line-height:1.75; font-weight:300; }
        ...
        .about-badge { position:absolute; top:2rem; left:-2rem; background:var(--gold); color:var(--ink); padding:1.2rem; border-radius:2px; text-align:center; box-shadow:0 8px 32px rgba(0,0,0,.4); }
        .about-badge-num { font-family:var(--serif); font-size:2rem; font-weight:600; line-height:1; display:block; }
        .about-badge-txt { font-size:.65rem; letter-spacing:.15em; text-transform:uppercase; margin-top:.3rem; display:block; }
        ...
        .stat-lbl { font-size:.75rem; color:var(--text-dim); letter-spacing:.1em; text-transform:uppercase; margin-top:.2rem; }
        ...
        .testi-card { background:var(--ink); padding:3rem 2.5rem; transition:background .35s; }
        .testi-card:hover { background:var(--ink-soft); }
        .testi-stars { display:flex; gap:2px; margin-bottom:1.5rem; color:var(--gold); font-size:.9rem; }
        .testi-quote { font-family:var(--serif); font-size:4rem; line-height:1; color:var(--gold); opacity:.25; margin-bottom:1.2rem; }
        ...
        .testi-name { font-size:.9rem; font-weight:500; margin-bottom:.15rem; }
        ...
        .faq-contact-card { background:var(--gold-pale); border:1px solid rgba(201,168,76,.25); border-radius:6px; padding:1.8rem; margin-top:2.5rem; }
        .fcc-label { font-size:.68rem; letter-spacing:.18em; text-transform:uppercase; color:var(--gold); margin-bottom:.8rem; }
        .fcc-name { font-family:var(--serif); font-size:1.4rem; margin-bottom:.2rem; }
        .fcc-role { font-size:.8rem; color:var(--text-dim); margin-bottom:1.2rem; }
        .fcc-phone { display:flex; align-items:center; gap:.5rem; color:var(--gold-light); font-weight:500; font-size:.95rem; transition:color .2s; }
        ...
        .faq-q { display:flex; align-items:center; justify-content:space-between; padding:1.6rem 0; gap:1rem; background:none; border:none; color:var(--cream); width:100%; text-align:left; font-size:1rem; font-family:var(--sans); transition:color .2s; }
        .faq-q:hover,.faq-q.active { color:var(--gold); }
        .faq-icon { width:28px; height:28px; flex-shrink:0; border:1px solid var(--mist-border); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:1rem; color:var(--text-dim); transition:transform .35s,border-color .25s,color .25s; }
        .faq-q.active .faq-icon { transform:rotate(45deg); border-color:var(--gold); color:var(--gold); }
        .faq-a { overflow:hidden; max-height:0; transition:max-height .45s var(--ease-out),padding .3s; }
        .faq-a.open { max-height:400px; padding-bottom:1.5rem; }
        .faq-a p { font-size:.92rem; color:var(--text-dim); line-height:1.8; font-weight:300; }
        ...
        .footer-brand p { font-size:.88rem; color:var(--text-dim); line-height:1.75; font-weight:300; max-width:260px; margin-top:1.2rem; }
        .footer-col h4 { font-size:.68rem; letter-spacing:.2em; text-transform:uppercase; color:var(--gold); margin-bottom:1.4rem; }
        .footer-col ul { list-style:none; display:flex; flex-direction:column; gap:.75rem; }
        .footer-col a { font-size:.88rem; color:var(--text-dim); transition:color .2s; }
        ...
        .policy-card h5 { font-size:.72rem; letter-spacing:.16em; text-transform:uppercase; color:var(--gold); margin-bottom:.6rem; }
        .policy-card p { font-size:.83rem; color:var(--text-dim); line-height:1.7; font-weight:300; }
        .footer-bottom { display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:1rem; }
        .footer-bottom p { font-size:.78rem; color:var(--text-dim); }
        .footer-bottom-links { display:flex; gap:1.5rem; }
        .footer-bottom-links a { font-size:.78rem; color:var(--text-dim); transition:color .2s; }
...
<nav class="mobile-nav" id="mobileNav" aria-label="Mobile navigation">
    ...
    <?php if ($isLoggedIn): ?>
        <a href="<?= htmlspecialchars($dashboardLink) ?>" style="color:var(--gold)">Dashboard &rarr;</a>
        ...
        <div class="container">
            <div class="hero-content">
                <?php if ($isLoggedIn): ?>
                    ...
                    <p class="hero-desc">Pick up where you left off and step back into a premium experience tailored to you.</p>
                <?php else: ?>
    ...
    <section class="features" aria-labelledby="features-heading">
        <div class="container">
            ...
            <div class="features-grid reveal reveal-delay-2">
                <div class="feature-card"><span class="feature-num">01</span><div class="feature-icon" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg></div><h3 class="feature-title">Expert Curation</h3><p class="feature-desc">Every item personally selected by our panel of luxury specialists with decades of combined experience.</p></div>
                <div class="feature-card"><span class="feature-num">02</span><div class="feature-icon" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg></div><h3 class="feature-title">100% Authentic</h3><p class="feature-desc">Direct brand partnerships and rigorous authentication. Every high-value item ships with a certificate of authenticity.</p></div>
                <div class="feature-card"><span class="feature-num">03</span><div class="feature-icon" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="1" y="3" width="15" height="13" rx="1"/><path d="M16 8h4l3 3v4h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg></div><h3 class="feature-title">White-Glove Delivery</h3><p class="feature-desc">Premium packaging, full tracking, and signature confirmation on every order.</p></div>
                <div class="feature-card"><span class="feature-num">04</span><div class="feature-icon" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg></div><h3 class="feature-title">Concierge Service</h3><p class="feature-desc">Our dedicated team assists with every step &mdash; selection, returns, exchanges, and care guidance.</p></div>
            </div>
        </div>
    </section>
        ...
        <div class="container">
            <div class="about-grid">
                ...
                <div class="about-text">
                    ...
                    <p class="section-subtitle reveal reveal-delay-3" style="margin-top:.8rem;">Every product passes through our rigorous selection process &mdash; because we believe what you own should reflect your standards.</p>
    ...
    <section id="testimonials" class="testimonials" aria-labelledby="testi-heading">
        <div class="container">
            ...
            <div class="testi-grid reveal reveal-delay-2">
                <?php
                $testimonials = [
                    ['name'=>'Sarah Johnson','role'=>'Fashion Designer, NY','initial'=>'S','text'=>"Exceptional quality that speaks before you do. Every purchase has been a lesson in what true craftsmanship looks like."],
        ...
        <div class="container">
            <div class="faq-layout">
                ...
                <div class="faq-list reveal reveal-delay-2">
                    <?php
                    $faqs = [
                        ['q'=>'What makes LuxeStore different?','a'=>'Our meticulous curation process sets us apart. Every product is personally selected by luxury experts focusing on timeless quality, craftsmanship, and enduring style.'],
                        ['q'=>'How do you ensure product authenticity?','a'=>'We work directly with brands and verified suppliers. Each product undergoes rigorous authentication and high-value items ship with a certificate of authenticity.'],
                        ['q'=>'What is your return and exchange policy?','a'=>'We offer a 30-day return window for items in original condition. Exchanges are available within 60 days. Our concierge team handles every return personally.'],
                        ['q'=>'Do you offer international shipping?','a'=>'Yes — we ship to over 50 countries using premium carriers with full tracking and signature confirmation. Customs duties are the recipient\'s responsibility.'],
...
<footer class="site-footer" id="site-footer" role="contentinfo">
    <div class="container">
        <div class="footer-top">
            ...
            <nav class="footer-col" aria-label="Navigate"><h4>Navigate</h4><ul><li><a href="#home">Home</a></li><li><a href="<?= htmlspecialchars($shopLink) ?>">Shop</a></li><li><a href="#about">About</a></li><li><a href="#faq">FAQ</a></li></ul></nav>
            <nav class="footer-col" aria-label="Support"><h4>Support</h4><ul><li><a href="#faq">FAQ</a></li><li><a href="<?= htmlspecialchars($isLoggedIn ? $dashboardLink : 'login.php') ?>">My Account</a></li><li><a href="mailto:<?= htmlspecialchars($contact_email) ?>"><?= htmlspecialchars($contact_email) ?></a></li><li><a href="tel:<?= htmlspecialchars($contact_phone) ?>"><?= htmlspecialchars($contact_phone) ?></a></li></ul></nav>
            <div class="footer-col"><h4>Visit Us</h4><address style="font-size:.88rem;color:var(--text-dim);line-height:1.7;">123 Luxury Avenue<br>Suite 100<br>New York, NY 10001</address></div>
        </div>
        <div class="footer-policies">
            <div class="policy-card"><h5>Shipping</h5><p>Orders processed within 1–2 business days with tracked delivery and signature confirmation.</p></div>
        ...
        <div class="footer-bottom">
            ...
            <div class="footer-bottom-links"><a href="#">Privacy Policy</a><a href="#">Terms of Service</a><a href="#">Cookie Settings</a></div>
        </div>
    </div>
</footer>
...
/*
 * KEY FIX 2 — Smooth anchor scroll WITHOUT history.pushState.
 *
 * The previous version called history.pushState on every anchor click,
 * which permanently appended the hash to the URL bar (showing #about, #faq).
 * Removing pushState entirely fixes this — the CSS scroll-padding-top on
 * <html> already handles the sticky-header offset, so no JS offset calc needed.
 */
document.querySelectorAll('a[href^="#"]').forEach(a => {
    a.addEventListener('click', e => {
        ...
        toggleNav(false);
        // Do NOT call history.pushState — it caused the #about URL pollution
    });
});
