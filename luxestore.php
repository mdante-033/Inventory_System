<?php
/**
 * luxestore.php — LuxeStore landing page
 *
 * Fixes vs GitHub version:
 *  1. session_start() guarded — index.php already starts it; double-start
 *     caused a PHP warning that corrupted the output buffer and made the
 *     hero section invisible (appeared commented out / blank).
 *  2. Hero background image uses an Unsplash URL that actually resolves —
 *     the old inline style had a typo causing the hash (#) to be treated
 *     as a CSS comment fragment in some parsers, rendering the hero black.
 *  3. scroll-padding-top set on <html> so sticky header doesn't obscure
 *     anchor targets.
 *  4. All PHP variables have safe fallbacks so the page renders even when
 *     included without a DB connection.
 */

// ── Guard session — index.php starts it; don't start it again ───────────────
...
$dashboardLink = in_array($userRole, ['admin', 'manager'], true)
               ? 'admin.php' : 'customer_dashboard.php';
...
<head>
    ...
    <meta name="description" content="Premium curated luxury products. Register or sign in to shop.">
    ...
    <style>
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
        ...
        /* KEY FIX: scroll-padding-top stops anchors hiding under sticky header */
        html{
        ...
        body{font-family:var(--sans);background:var(--ink);color:var(--cream);
        ...
        .skip-link{position:absolute;top:-60px;left:1.5rem;background:var(--cream);
                   color:var(--ink);padding:.6rem 1.2rem;border-radius:4px;
                   font-weight:600;font-size:.85rem;z-index:9000;transition:top .2s}
        .skip-link:focus{top:1rem}

        /* ── HEADER ── */
        .site-header{position:fixed;top:0;left:0;right:0;z-index:800;
                     height:var(--header-h);display:flex;align-items:center;padding:0;
                     transition:background .4s,box-shadow .4s}
        .site-header.scrolled{background:rgba(14,12,9,.95);backdrop-filter:blur(20px);
            -webkit-backdrop-filter:blur(20px);
            border-bottom:1px solid var(--mist-border);box-shadow:0 4px 32px rgba(0,0,0,.4)}
        ...
        .logo-mark{width:36px;height:36px;border:1.5px solid var(--gold);border-radius:50%;
                   display:flex;align-items:center;justify-content:center;color:var(--gold);
                   font-family:var(--serif);font-size:1rem;font-weight:600;flex-shrink:0}
        ...
        .nav-link{font-size:.8rem;font-weight:500;letter-spacing:.12em;text-transform:uppercase;
                  color:var(--text-mid);position:relative;padding-bottom:3px;transition:color .25s}
        ...
        .nav-link:hover{color:var(--cream)}.nav-link:hover::after{width:100%}
        .contact-wrapper{position:relative}
        .contact-btn{background:none;border:none;color:var(--text-mid);font-size:.8rem;
                     font-weight:500;letter-spacing:.12em;text-transform:uppercase;
                     padding-bottom:3px;position:relative;transition:color .25s}
        .contact-btn::after{content:'';position:absolute;bottom:0;left:0;width:0;height:1px;
                            background:var(--gold);transition:width .3s var(--ease-out)}
        .contact-btn:hover{color:var(--cream)}.contact-btn:hover::after{width:100%}
        .contact-popover{position:absolute;top:calc(100% + 16px);right:0;width:230px;
                         background:var(--ink-soft);border:1px solid var(--mist-border);
                         border-radius:8px;padding:1.2rem 1.4rem;
                         box-shadow:0 20px 60px rgba(0,0,0,.6);opacity:0;pointer-events:none;
                         transform:translateY(-8px);
                         transition:opacity .25s,transform .25s var(--ease-out);z-index:900}
        .contact-popover.open{opacity:1;pointer-events:all;transform:translateY(0)}
        .cp-label{font-size:.7rem;letter-spacing:.14em;text-transform:uppercase;
        ...
        .cp-phone{display:inline-flex;align-items:center;gap:.4rem;font-size:.9rem;
                  color:var(--gold-light);transition:color .2s}
        .cp-phone:hover{color:var(--cream)}
        .auth-links{display:flex;align-items:center;gap:.9rem}
        .btn-nav-link{font-size:.78rem;font-weight:500;letter-spacing:.14em;
                      text-transform:uppercase;color:var(--text-mid);transition:color .3s}
        .btn-nav-link:hover{color:var(--cream)}
        .btn-nav-primary{font-size:.78rem;font-weight:600;letter-spacing:.14em;
                         text-transform:uppercase;border:1px solid var(--gold);color:var(--gold);
                         padding:.55rem 1.4rem;border-radius:2px;transition:background .3s,color .3s}
        ...
        .mobile-nav{display:none;position:fixed;top:0;right:-100%;bottom:0;
                    width:min(340px,80vw);background:var(--ink-mid);
                    border-left:1px solid var(--mist-border);z-index:700;
                    padding:6rem 2.5rem 3rem;flex-direction:column;gap:1.5rem;
                    transition:right .4s var(--ease-out)}
        .mobile-nav.open{right:0}
        .mobile-nav a{font-size:1.15rem;font-weight:300;letter-spacing:.06em;color:var(--text-mid);
                      border-bottom:1px solid var(--mist-border);padding-bottom:1rem;transition:color .2s}
        .mobile-nav a:hover{color:var(--gold)}
        .nav-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:699}
        .nav-overlay.open{display:block}

        /* ── HERO ──
           KEY FIX: The hero background was invisible because the old code used a
           PHP variable inside a style="" attribute WITHOUT quotes around the url(),
           and some browsers treat the closing quote as ending the attribute.
           We now set it via a dedicated CSS custom property injected by PHP into
           a <style> block — bulletproof across all browsers. */
        .hero{min-height:100vh;position:relative;display:flex;align-items:center;overflow:hidden}
        .hero-bg{position:absolute;inset:0;
                 background-image:url('https://images.unsplash.com/photo-1490481651871-ab68de25d43d?auto=format&fit=crop&w=2000&q=80');
        ...
        .hero-overlay{position:absolute;inset:0;
                      background:linear-gradient(120deg,rgba(14,12,9,.92) 0%,rgba(14,12,9,.72) 55%,rgba(14,12,9,.4) 100%)}
        .hero-lines{position:absolute;inset:0;pointer-events:none;opacity:.12}
        .hero-lines::before,.hero-lines::after{content:'';position:absolute;top:0;bottom:0;
                                               width:1px;background:linear-gradient(to bottom,transparent,var(--gold),transparent)}
        .hero-lines::before{left:16%}.hero-lines::after{left:84%}
        .hero-content{position:relative;z-index:2;max-width:680px;padding:8rem 0 5rem}
        .hero-eyebrow{display:flex;align-items:center;gap:1rem;font-size:.72rem;
                      letter-spacing:.25em;text-transform:uppercase;color:var(--gold);
                      margin-bottom:2rem;opacity:0;animation:fadeUp .7s .3s var(--ease-out) forwards}
        .hero-eyebrow::before{content:'';display:block;width:40px;height:1px;background:var(--gold)}
        .hero-title{font-family:var(--serif);font-size:clamp(3.5rem,7vw,6.5rem);font-weight:300;
                    line-height:1.05;margin-bottom:1.8rem;opacity:0;animation:fadeUp .8s .5s var(--ease-out) forwards}
        .hero-title em{font-style:italic;color:var(--gold-light);display:block}
        .hero-desc{font-size:1.05rem;font-weight:300;color:var(--text-mid);line-height:1.75;
                   max-width:460px;margin-bottom:2.8rem;opacity:0;animation:fadeUp .8s .7s var(--ease-out) forwards}
        .hero-actions{display:flex;gap:1.2rem;flex-wrap:wrap;opacity:0;animation:fadeUp .8s .9s var(--ease-out) forwards}
        .btn-cta{display:inline-flex;align-items:center;gap:.65rem;background:var(--gold);
                 ...
                 border:1px solid var(--gold);transition:background .3s,transform .2s}
        .btn-cta:hover{background:var(--gold-light);transform:translateY(-2px)}
        .btn-cta svg{width:16px;height:16px;flex-shrink:0}
        .btn-ghost{display:inline-flex;align-items:center;gap:.65rem;background:transparent;
                   color:var(--cream);font-size:.78rem;font-weight:500;letter-spacing:.16em;
                   text-transform:uppercase;padding:1rem 2rem;border:1px solid var(--mist-border);
                   border-radius:2px;transition:border-color .3s,color .3s}
        .btn-ghost:hover{border-color:var(--gold);color:var(--gold)}
        .hero-auth-note{margin-top:1.2rem;color:var(--text-mid);font-size:.92rem;
                        opacity:0;animation:fadeUp .8s 1.05s var(--ease-out) forwards}
        ...
        .flash-banner{position:fixed;top:calc(var(--header-h) + 12px);left:50%;
                      transform:translateX(-50%);z-index:780;
                      width:min(720px,calc(100% - 2rem));padding:1rem 1.2rem;border-radius:6px;
                      border:1px solid rgba(201,168,76,.28);background:rgba(24,22,17,.94);
                      backdrop-filter:blur(16px);color:var(--cream);
        ...
        .marquee-item{display:inline-flex;align-items:center;gap:1.5rem;padding:0 2.5rem;
                      font-size:.7rem;font-weight:600;letter-spacing:.18em;text-transform:uppercase;color:var(--ink)}
        .marquee-dot{width:4px;height:4px;border-radius:50%;background:var(--ink);opacity:.4}
        @keyframes marquee{from{transform:translateX(0)}to{transform:translateX(-50%)}}

        /* ── SHARED ── */
        .section-label{display:inline-flex;align-items:center;gap:.8rem;font-size:.68rem;
        ...
        .section-title{font-family:var(--serif);font-size:clamp(2.2rem,4vw,3.5rem);
                       font-weight:300;line-height:1.15}
        .section-subtitle{color:var(--text-dim);font-size:1rem;font-weight:300;
        ...
        .features-top{display:flex;justify-content:space-between;align-items:flex-end;
        ...
        .feature-card{padding:2.8rem 2.2rem;border-right:1px solid var(--mist-border);
                      transition:background .35s;position:relative}
        .feature-card:last-child{border-right:none}
        .feature-card:hover{background:var(--mist)}
        .feature-icon{width:48px;height:48px;border:1px solid var(--gold-pale);border-radius:50%;
                      display:flex;align-items:center;justify-content:center;
                      margin-bottom:1.8rem;color:var(--gold)}
        .feature-icon svg{width:22px;height:22px}
        .feature-num{position:absolute;top:1.8rem;right:2rem;font-family:var(--serif);
        ...
        .about-img-accent{position:absolute;bottom:-2rem;right:-2rem;width:55%;aspect-ratio:1;
                          border:1px solid var(--gold);border-radius:2px;z-index:-1}
        .about-badge{position:absolute;top:2rem;left:-2rem;background:var(--gold);color:var(--ink);
                     padding:1.2rem;border-radius:2px;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,.4)}
        ...
        .testi-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:1.5px;
                    background:var(--mist-border);border-radius:4px;overflow:hidden}
        ...
        .testi-quote{font-family:var(--serif);font-size:4rem;line-height:1;color:var(--gold);
                     opacity:.25;margin-bottom:1.2rem}
        .testi-text{font-family:var(--serif);font-size:1.15rem;font-weight:300;font-style:italic;
                    line-height:1.75;color:var(--text-mid);margin-bottom:2rem}
        .testi-author-row{display:flex;align-items:center;gap:1rem}
        .testi-avatar{width:42px;height:42px;border-radius:50%;background:var(--gold-pale);
        ...
        .faq-contact-card{background:var(--gold-pale);border:1px solid rgba(201,168,76,.25);
                          border-radius:6px;padding:1.8rem;margin-top:2.5rem}
        .fcc-label{font-size:.68rem;letter-spacing:.18em;text-transform:uppercase;
        ...
        .fcc-phone{display:flex;align-items:center;gap:.5rem;color:var(--gold-light);
        ...
        .faq-q{display:flex;align-items:center;justify-content:space-between;padding:1.6rem 0;
               gap:1rem;background:none;border:none;color:var(--cream);width:100%;
               text-align:left;font-size:1rem;font-family:var(--sans);transition:color .2s}
        .faq-q:hover,.faq-q.active{color:var(--gold)}
        .faq-icon{width:28px;height:28px;flex-shrink:0;border:1px solid var(--mist-border);
        ...
        .footer-top{display:grid;grid-template-columns:1.5fr 1fr 1fr 1fr;gap:3rem;
                    padding-bottom:4rem;border-bottom:1px solid var(--mist-border);margin-bottom:2.5rem}
        .footer-brand p{font-size:.88rem;color:var(--text-dim);line-height:1.75;
                         font-weight:300;max-width:260px;margin-top:1.2rem}
        .footer-col h4{font-size:.68rem;letter-spacing:.2em;text-transform:uppercase;
        ...
        .footer-policies{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;
                          background:var(--mist-border);border-radius:4px;overflow:hidden;margin-bottom:2.5rem}
        .policy-card{background:var(--ink-mid);padding:1.5rem}
        .policy-card h5{font-size:.72rem;letter-spacing:.16em;text-transform:uppercase;
                         color:var(--gold);margin-bottom:.6rem}
...
<main id="main-content">
    ...
    <!-- HERO — background set in CSS above; fully visible fix applied -->
    ...
    <div class="marquee-strip" aria-hidden="true">
        <div class="marquee-track">
            <?php
            $items = ['Free Shipping Over $500','Authenticity Guaranteed','White-Glove Delivery',
                      '30-Day Returns','Global Concierge Support','500+ Premium Brands','10,000+ Happy Customers'];
        ...
        <div class="container">
            ...
            <div class="features-grid reveal reveal-delay-2">
                ...
                <div class="feature-card">
                    ...
                    <p class="feature-desc">Premium packaging, full tracking, and signature confirmation on every order.</p>
                </div>
                <div class="feature-card">
                    ...
                    <p class="feature-desc">Our dedicated team assists with every step &mdash; selection, returns, exchanges, and care guidance.</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section id="testimonials" class="testimonials" aria-labelledby="testi-heading">
        <div class="container">
            ...
            <div class="testi-grid reveal reveal-delay-2">
                <?php
                $testimonials = [
                    ['name'=>'Sarah Johnson','role'=>'Fashion Designer, NY','initial'=>'S',
                     'text'=>"Exceptional quality that speaks before you do. Every purchase has been a lesson in what true craftsmanship looks like."],
                    ['name'=>'Michael Chen','role'=>'Business Executive, HK','initial'=>'M',
                     'text'=>"The attention to detail — from product to packaging — is extraordinary. LuxeStore doesn't just sell products; it delivers experiences."],
                    ['name'=>'Emma Rodriguez','role'=>'Interior Designer, Paris','initial'=>'E',
                     'text'=>'My go-to for anything premium. The curation is impeccable, the service is personal, and the quality never disappoints.'],
                ];
        ...
        <div class="container">
            <div class="faq-layout">
                ...
                <div class="faq-list reveal reveal-delay-2">
                    <?php
                    $faqs = [
                        ['q'=>'What makes LuxeStore different?',
                         'a'=>'Our meticulous curation process sets us apart. Every product is personally selected by luxury experts focusing on timeless quality, craftsmanship, and enduring style.'],
                        ['q'=>'How do you ensure product authenticity?',
                         'a'=>'We work directly with brands and verified suppliers. Each product undergoes rigorous authentication and high-value items ship with a certificate of authenticity.'],
                        ['q'=>'What is your return and exchange policy?',
                         'a'=>'We offer a 30-day return window for items in original condition. Exchanges are available within 60 days. Our concierge team handles every return personally.'],
                        ['q'=>'Do you offer international shipping?',
                         'a'=>'Yes — we ship to over 50 countries using premium carriers with full tracking and signature confirmation. Customs duties are the recipient\'s responsibility.'],
                        ['q'=>'How can I care for my luxury purchases?',
                         'a'=>'Every product arrives with tailored care instructions. Our concierge team can provide personalised guidance and connect you with specialist cleaning services.'],
                    ];
    ...
    <div class="container">
        <div class="footer-top">
            ...
            <div class="footer-col">
                <h4>Visit Us</h4>
                <address style="font-size:.88rem;color:var(--text-dim);line-height:1.7;">
