...
/* ── Output buffer MUST be started, not cleaned ── */
ob_start();
...
/* ── JSON body merge (supports both form-encoded AND JSON fetch) ── */
...
/* ── Helper: is this a POST/AJAX request? ── */
function _isPostOrAjax(): bool {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') return true;
    return strtolower($_SERVER['HTTP_X_REQUESTED_WITH'] ?? '') === 'xmlhttprequest';
}

/* ── Helper: send JSON error and exit (for AJAX-safe auth failures) ── */
function _jsonAuthError(string $message, int $code = 401): void {
...
/* ── checkAdminPermission fallback — MUST come before auth-check.php ── */
    ...
    function checkAdminPermission(string $permission): bool {
        ...
        $map = [
            'manager' => [
                ...
                'categories.manage','stock.manage',
                'orders.manage','reports.view','reports.export','analytics.view',
            ],
            'staff' => [
                'products.view','products.add','products.edit',
                'stock.view','stock.manage','orders.view','orders.create',
            ],
        ];
        $perms = $map[$role] ?? [];
...
if (!isset($pdo) || $pdo === null) {
    if (_isPostOrAjax()) {
        _jsonAuthError('Database connection failed.', 503);
    }
...
try {
    ...
    if ($currentAdminId > 0) {
        ...
        if (!$sessionUser) {
            destroySession();
            if (_isPostOrAjax()) _jsonAuthError('Session expired. Please log in again.', 401);
        ...
        if (!in_array($sessionUser['role'] ?? '', ['admin', 'manager'], true)
            ...
            destroySession();
            if (_isPostOrAjax()) _jsonAuthError('Access denied. Please log in again.', 403);
            header('Location: login.php?error=unauthorized'); exit;
        }
    ...
    try {
        switch ($action) {
            ...
            case 'add_user': {
                ...
                if ($username === '' || $email === '' || $fullName === '' || $passwordInput === '') { $result = ['success' => false, 'message' => 'Please complete all required user fields.']; break; }
                if (!filter_var($email, FILTER_VALIDATE_EMAIL)) { $result = ['success' => false, 'message' => 'Please enter a valid email address.']; break; }
                if (strtolower($username) === strtolower($email)) { $result = ['success' => false, 'message' => 'Username and email must be different.']; break; }
                if (!in_array($role, $allowedRoles, true)) { $result = ['success' => false, 'message' => 'Please select a valid user role.']; break; }
            ...
            case 'update_user': {
                ...
                if ($userId === false || $username === '' || $email === '' || $fullName === '') { $result = ['success' => false, 'message' => 'Please complete all required user fields.']; break; }
                if (!filter_var($email, FILTER_VALIDATE_EMAIL)) { $result = ['success' => false, 'message' => 'Please enter a valid email address.']; break; }
                if (strtolower($username) === strtolower($email)) { $result = ['success' => false, 'message' => 'Username and email must be different.']; break; }
                if (!in_array($role, $allowedRoles, true)) { $result = ['success' => false, 'message' => 'Please select a valid user role.']; break; }
                if ($currentUserId > 0 && $currentUserId === $userId && !$isActive) { $result = ['success' => false, 'message' => 'You cannot deactivate your own account.']; break; }
                ...
                if (!in_array($existingUser['role'] ?? '', $allowedRoles, true)) { $result = ['success' => false, 'message' => 'Only staff, manager, and admin accounts can be edited here.']; break; }
...
<head>
        ...
        window.fetch = function (input, init) {
            ...
            if (method === 'POST' && window.ADMIN_CSRF_TOKEN) {
                if (options.body instanceof FormData || options.body instanceof URLSearchParams) {
                    if (!options.body.has('csrf_token')) options.body.append('csrf_token', window.ADMIN_CSRF_TOKEN);
    ...
    :root {
        color-scheme: light;
        --bg:#f4f7fb;--bg-2:#ebf0f7;--bg-3:#dde7f3;--bg-4:#d2dceb;
        --surface:#ffffff;--surface-2:#f8fbff;
        --accent:#6366f1;--accent-glow:rgba(99,102,241,.18);--accent-dim:rgba(99,102,241,.12);
        --accent-2:#22d3ee;--accent-3:#f59e0b;--accent-4:#10b981;--accent-5:#f43f5e;
        --text:#172033;--text-2:#556177;--text-3:#7d8798;
        --border:rgba(23,32,51,.10);--border-2:rgba(23,32,51,.16);
        --success:#10b981;--warning:#f59e0b;--danger:#f43f5e;--info:#22d3ee;
        --success-bg:rgba(16,185,129,.12);--warning-bg:rgba(245,158,11,.12);
        --danger-bg:rgba(244,63,94,.12);--info-bg:rgba(34,211,238,.12);
        --primary:var(--accent);--secondary:var(--accent-4);
        --bg-card:var(--surface);--bg-main:var(--bg-3);
        ...
        --border-radius-sm:8px;--border-radius-md:14px;
        --success-light:var(--success-bg);--warning-light:var(--warning-bg);
        --danger-light:var(--danger-bg);--info-light:var(--info-bg);
        --shadow-md:0 14px 32px rgba(15,23,42,.08);--shadow-lg:0 20px 48px rgba(15,23,42,.12);
        --header-bg:rgba(255,255,255,.82);
        --sidebar-w:240px;--sidebar-collapsed:68px;--header-h:64px;
        --radius:12px;--radius-lg:18px;
        --theme-shadow-focus:0 0 0 3px rgba(99,102,241,.18);--theme-transition:.32s ease;
    }
    :root[data-theme="dark"] {
        color-scheme:dark;
        --bg:#0d0f14;--bg-2:#12151c;--bg-3:#181c26;--bg-4:#1e2330;
        --surface:#242938;--surface-2:#2c3244;
        --accent:#6366f1;--accent-glow:rgba(99,102,241,.28);--accent-dim:rgba(99,102,241,.12);
        --accent-2:#22d3ee;--accent-3:#f59e0b;--accent-4:#10b981;--accent-5:#f43f5e;
        --text:#e8eaf0;--text-2:#9ba3b8;--text-3:#5b6480;
        ...
        --text-primary:var(--text);--text-secondary:var(--text-2);--text-muted:var(--text-3);
        --border-color:var(--border);
        --shadow-md:0 4px 20px rgba(0,0,0,.4);--shadow-lg:0 8px 40px rgba(0,0,0,.5);
        --gradient-primary:linear-gradient(135deg,var(--accent),var(--accent-2));
        --border-radius-sm:8px;--border-radius-md:14px;
        --success-light:var(--success-bg);--warning-light:var(--warning-bg);
        --danger-light:var(--danger-bg);--info-light:var(--info-bg);
        ...
        --sidebar-w:240px;--sidebar-collapsed:68px;--header-h:64px;
        --radius:12px;--radius-lg:18px;
        --theme-shadow-focus:0 0 0 3px rgba(99,102,241,.28);--theme-transition:.32s ease;
    }
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    ::-webkit-scrollbar{width:5px;height:5px}::-webkit-scrollbar-track{background:var(--bg-2)}::-webkit-scrollbar-thumb{background:var(--surface-2);border-radius:99px}
    a{color:inherit;text-decoration:none}button{font-family:inherit}input,select,textarea{font-family:inherit}
    ...
    .nav-link:hover{background:var(--surface);color:var(--text)}.nav-link:hover i{color:var(--accent)}
    .nav-link.active{background:var(--accent-dim);color:var(--accent);box-shadow:inset 3px 0 0 var(--accent)}.nav-link.active i{color:var(--accent)}
    ...
    .sidebar.collapsed .sidebar-toggle i{transform:rotate(180deg)}.sidebar.collapsed .sidebar-toggle span{opacity:0}
    ...
    .page-title .crumb{color:var(--text-3);font-weight:400}.page-title .sep{color:var(--text-3)}
    ...
    .notification-item{padding:10px 12px;border-bottom:1px solid var(--border);font-size:.8rem;color:var(--text-2)}.notification-item:last-child{border-bottom:none}.notification-item a{color:var(--text)}
    ...
    .user-name{font-size:.8rem;font-weight:600;color:var(--text);line-height:1.2}.user-role{font-size:.7rem;color:var(--text-3);text-transform:capitalize}
    ...
    .card-title{font-size:.9rem;font-weight:600;color:var(--text);display:flex;align-items:center;gap:8px}.card-title i{color:var(--accent);font-size:15px}
    .card-body{padding:24px}.card-footer{padding:16px 24px;border-top:1px solid var(--border);background:var(--bg-3)}
    ...
    .stat-card:hover{transform:translateY(-3px);box-shadow:0 12px 36px rgba(0,0,0,.3);border-color:var(--border-2)}.stat-card:hover::before{opacity:1}
    ...
    .stat-content{flex:1}.stat-value{font-size:1.7rem;font-weight:700;color:var(--text);letter-spacing:-.03em;line-height:1.1}
    ...
    .stat-trend.up{color:var(--success)}.stat-trend.down{color:var(--danger)}.stat-trend.neutral{color:var(--text-3)}
    ...
    .quick-action-btn i{font-size:20px;color:var(--accent)}.quick-action-btn span{font-size:.78rem;font-weight:600;color:var(--text-2)}
    ...
    .top-product-info{flex:1;min-width:0}.top-product-name{font-weight:600;font-size:.82rem;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .top-product-sales{font-size:.75rem;color:var(--text-3)}.top-product-revenue{font-weight:600;color:var(--success);font-size:.82rem;font-family:'DM Mono',monospace}
    .table-wrap{overflow-x:auto}.data-table{width:100%;border-collapse:collapse}
    ...
    .data-table tbody tr:hover td{background:var(--bg-3);color:var(--text)}.data-table tbody tr:last-child td{border-bottom:none}
    ...
    .badge-info{background:var(--info-bg);color:var(--info)}.badge-danger{background:var(--danger-bg);color:var(--danger)}
    ...
    .btn-primary{background:var(--accent);color:#fff;box-shadow:0 4px 16px var(--accent-glow)}.btn-primary:hover{background:#7577f3;box-shadow:0 6px 22px var(--accent-glow);transform:translateY(-1px)}
    .btn-secondary{background:var(--surface-2);color:var(--text-2);border:1px solid var(--border)}.btn-secondary:hover{background:var(--bg-4);color:var(--text)}
    .btn-outline{background:transparent;color:var(--text-2);border:1px solid var(--border)}.btn-outline:hover{background:var(--surface-2);color:var(--text);border-color:var(--border-2)}
    .btn-outline.active{background:var(--accent-dim);color:var(--accent);border-color:var(--accent)}
    .btn-danger{background:var(--danger-bg);color:var(--danger);border:1px solid rgba(244,63,94,.2)}.btn-danger:hover{background:var(--danger);color:#fff}
    .btn-sm{padding:6px 13px;font-size:.76rem}.btn-xs{padding:4px 10px;font-size:.72rem}
    ...
    .form-group input,.form-group select,.form-group textarea{width:100%;padding:10px 14px;background:var(--bg-3);border:1px solid var(--border);border-radius:var(--radius);font-size:.85rem;color:var(--text);transition:border-color .2s,box-shadow .2s;outline:none}
    ...
    .form-group select option{background:var(--bg-3)}.form-group textarea{resize:vertical;min-height:90px}
    ...
    .action-icon:hover{background:var(--surface-2);color:var(--accent)}.action-icon.delete:hover{background:var(--danger-bg);color:var(--danger)}
    ...
    .modal-body{padding:24px}.modal-footer{display:flex;justify-content:flex-end;gap:10px;padding:20px 24px;border-top:1px solid var(--border)}
    ...
    .nav-tab:hover{color:var(--text);background:var(--bg-3)}.nav-tab.active{color:var(--accent);border-bottom-color:var(--accent);background:var(--accent-dim)}
    ...
    .settings-tab:hover{background:var(--surface-2);color:var(--text)}.settings-tab.active{background:var(--accent-dim);border-color:var(--accent);color:var(--accent)}
    ...
    .report-icon.sales{background:rgba(99,102,241,.15);color:var(--accent)}.report-icon.inventory{background:rgba(16,185,129,.15);color:var(--success)}
    .report-icon.products{background:rgba(245,158,11,.15);color:var(--warning)}.report-icon.customers{background:rgba(34,211,238,.15);color:var(--info)}
    .report-info h4{font-size:.9rem;font-weight:600;margin-bottom:4px;color:var(--text)}.report-info p{font-size:.8rem;color:var(--text-3);margin-bottom:10px;line-height:1.5}
    ...
    .export-buttons{display:flex;gap:10px;flex-wrap:wrap}.date-range-picker{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px}
    .page-section{display:none}.page-section.active{display:block}
    .page-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;flex-wrap:wrap;gap:12px}
    .page-header h2{font-size:1.3rem;font-weight:700;letter-spacing:-.02em;color:var(--text)}.page-header p{font-size:.82rem;color:var(--text-3);margin-top:2px}
    ...
    .empty-state i{font-size:40px;opacity:.4}.empty-state p{font-size:.85rem}
    ...
    .mono{font-family:'DM Mono',monospace}.text-muted{color:var(--text-3)}.text-success{color:var(--success)}.text-danger{color:var(--danger)}.text-warning{color:var(--warning)}.text-accent{color:var(--accent)}
    .flex{display:flex}.items-center{align-items:center}.gap-2{gap:8px}.gap-3{gap:12px}
    .mt-1{margin-top:4px}.mt-2{margin-top:8px}.mb-3{margin-bottom:12px}.mb-4{margin-bottom:16px}.mb-6{margin-bottom:24px}.w-full{width:100%}
    @media(max-width:768px){
        .sidebar{transform:translateX(-100%);width:var(--sidebar-w)}.sidebar.mobile-open{transform:translateX(0)}
        ...
        .main-content{padding:16px}.header{padding:0 16px}.search-box{display:none}
        .quick-actions{grid-template-columns:repeat(2,1fr)}.top-products{grid-template-columns:1fr}
    }
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            <ul class="nav">
                ...
                foreach ($nav as $key => $info):
                    if ($key === '_divider'): ?><div class="nav-divider"></div><?php continue; endif; ?>
                    <li class="nav-item">
        ...
        <div class="sidebar-footer">
            <button class="sidebar-toggle" onclick="toggleSidebar()">
                <i class="fas fa-chevrons-left"></i><span>Collapse</span>
            </button>
    ...
    <div class="main-wrapper" id="mainWrapper">
        <header class="header">
            <div class="header-left">
                ...
                <h1 class="page-title">
                    <span class="crumb">StockFlow</span><span class="sep">/</span><?= htmlspecialchars($page_title) ?>
                </h1>
            </div>
            <div class="header-right">
                ...
                <button type="button" class="header-icon-btn theme-toggle" id="themeToggle" aria-label="Toggle color theme" aria-pressed="false" title="Toggle theme">
        ...
        <main class="main-content" id="mainContent">
            ...
            if (file_exists($module_file)) {
                ...
                echo "<div class='card'><div class='card-body'><div class='empty-state'>";
...
function getSavedTheme() { const t = localStorage.getItem(storageKey); return (t==='light'||t==='dark')?t:null; }
...
typeof systemTheme.addEventListener==='function' ? systemTheme.addEventListener('change',_mq) : systemTheme.addListener?.(_mq);
...
document.addEventListener('click', e => {
    ...
    if (window.innerWidth<=768 && sidebar.classList.contains('mobile-open')) {
        if (!sidebar.contains(e.target) && e.target!==btn && !btn?.contains(e.target)) sidebar.classList.remove('mobile-open');
    }
});
...
function hideNotification() { document.getElementById('notificationToast').style.display = 'none'; }
window.showNotification = showNotification;
window.hideNotification = hideNotification;

function showModal(id) { const el=document.getElementById(id); if(!el)return; el.classList.add('active'); document.body.style.overflow='hidden'; }
function hideModal(id) { const el=document.getElementById(id); if(!el)return; el.classList.remove('active'); document.body.style.overflow=''; }
